...
def get_audit_log(
    ...
    Filters:
      ...
      - allowed: true = only allowed requests,
                 false = only blocked requests
...
def get_stats():
    ...
    total_requests = len(
        audit_logger.query(limit=100000)
    )
...
def get_access_control_lists():
    """
    Return the current allowlist and blocklist.
    """
...
def add_to_blocklist(body: AccessControlRequest):
    if body.entity_type == "user":
        ...
        return JSONResponse(
            status_code=400,
            content={
                "error": "entity_type must be 'user' or 'ip'"
            },
        )
